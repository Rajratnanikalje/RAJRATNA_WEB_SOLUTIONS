...
export async function updateAccount(req, res) {
  ...
  if (!email && !newPassword)
    return res.status(400).json({ message: "Enter a new email or password" });
  if (!/^[a-f0-9]{64}$/i.test(resetToken))
    return res
      .status(401)
      .json({ message: "Verify the email OTP before updating your account." });
  ...
  if (!admin)
    return res
      .status(401)
      .json({
        message: "Your OTP verification has expired. Please request a new OTP.",
      });
...
export async function requestPasswordReset(req, res) {
  ...
  if (!hasSmtpConfiguration())
    return res
      .status(503)
      .json({ message: "Password reset email is not configured yet." });
  const admin = await Admin.findOne({ email, active: true }).select(
    "+passwordResetOtpSentAt",
  );
  if (!admin)
    return res.json({ message: genericResetMessage, resendAfter: 60 });

  const elapsed = admin.passwordResetOtpSentAt
    ? Date.now() - admin.passwordResetOtpSentAt.getTime()
    : resendCooldownMs;
...
export async function verifyPasswordResetOtp(req, res) {
  ...
  if (!emailPattern.test(email) || !/^\d{6}$/.test(otp))
    return res
      .status(422)
      .json({ message: "Enter a valid email address and 6-digit OTP." });
  const admin = await Admin.findOne({ email, active: true }).select(
    "+passwordResetOtpHash +passwordResetOtpExpires +passwordResetOtpAttempts +passwordResetOtpSentAt",
  );
  if (
    !admin ||
    ...
    return res
      ...
      .json({
        message:
          "This OTP is invalid or has expired. Please request a new one.",
      });
  ...
  const valid = crypto.timingSafeEqual(
    ...
    Buffer.from(admin.passwordResetOtpHash, "hex"),
  );
  if (!valid) {
