...
export default function Footer() {
  ...
  const email =
    settings.footerEmail || settings.email || "rajratnawebsolutions@gmail.com";
  const location =
  ...
  const whatsappHref = withWhatsAppMessage(
    /^https?:\/\//i.test(whatsappValue)
      ? whatsappValue
      : settings.contactWhatsappUrl || digits,
  );
  const socialLinks = (
    Array.isArray(settings.footerSocialLinks)
      ? settings.footerSocialLinks
      : Array.isArray(settings.socialLinks)
        ? settings.socialLinks
        : []
  ).filter((url) => /^https?:\/\//i.test(url));
  const quickLinks = (settings.footerQuickLinks || defaultQuickLinks)
    .filter((item) => item.visible !== false)
    .sort((a, b) => (a.displayOrder || 0) - (b.displayOrder || 0));
  const serviceLinks = (settings.footerServiceLinks || defaultServiceLinks)
    .filter((item) => item.visible !== false)
    .sort((a, b) => (a.displayOrder || 0) - (b.displayOrder || 0));
  const copyright = (
    settings.footerCopyrightText ||
    "© {year} RAJRATNA WEB SOLUTIONS. All rights reserved."
  )
    .replaceAll("{year}", String(new Date().getFullYear()))
    .replaceAll("{siteTitle}", siteTitle);
  const columns = [
    settings.footerBrandVisible !== false,
    settings.footerQuickLinksVisible !== false,
    settings.footerServicesVisible !== false,
    settings.footerContactVisible !== false,
    ...
    <footer className="border-t border-white/5 pt-14">
      <div className="container">
        <div className={`grid sm:grid-cols-2 ${gridClass} gap-10`}>
          {settings.footerBrandVisible !== false && (
            <div>
              ...
              >
                ...
                <span>
                  <b className="block text-xl font-bold text-white">
                    {settings.brandPrimaryText || "RAJRATNA ."}
                  </b>
                  <span className="text-[9px] tracking-[.13em] text-slate-500">
                    {settings.brandSecondaryText || "WEB SOLUTIONS"}
                  </span>
                </span>
              </Link>
              ...
              {settings.footerSocialVisible !== false &&
                !!socialLinks.length && (
          ...
          {settings.footerQuickLinksVisible !== false && (
            <div>
              <b className="block text-sm font-medium text-slate-400 mb-3">
                {settings.footerQuickLinksHeading || "Pages"}
              </b>
              <div className="flex flex-col gap-2 text-sm">
                {quickLinks.map((item, index) => (
                  <Link
                    key={`${item.routeKey}-${index}`}
                    to={publicPath(item.routeKey)}
                    onClick={
                      item.routeKey === "home"
                        ? () =>
                            window.scrollTo({
                  ...
                  >
                    {item.routeKey === "privacy"
                      ? settings.footerPrivacyLabel || item.label
                      : item.routeKey === "terms"
                        ? settings.footerTermsLabel || item.label
                        : item.label}
                  </Link>
                ))}
              </div>
          ...
          {settings.footerServicesVisible !== false && (
            <div>
              <b className="block text-sm font-medium text-slate-400 mb-3">
                {settings.footerServicesHeading || "What We Build"}
              </b>
              ...
              <div className="flex flex-col gap-3 text-sm">
                {settings.footerShowPhone !== false && (
                  <a
                    ...
                    className="flex gap-2 text-slate-300 hover:text-white"
                  >
                    <Phone size={14} className="text-[#78a9ff] shrink-0" />
                    <span>{phone}</span>
                  </a>
                )}
                {settings.footerShowEmail !== false && (
                  <a
                    href={`mailto:${email}`}
                    className="flex gap-2 text-slate-300 hover:text-white break-all"
                  >
                    <Mail size={14} className="text-[#78a9ff] shrink-0" />
                    <span>{email}</span>
                  </a>
                )}
                {settings.footerShowWhatsapp !== false && whatsappHref && (
                  <a
                    href={whatsappHref}
                    target="_blank"
                    rel="noreferrer"
                  ...
                  >
                    ...
                    <span>{settings.footerWhatsappLabel || "WhatsApp"}</span>
                  </a>
                )}
        ...
        <div className="border-t border-white/5 mt-12 pt-6 text-xs text-slate-500 flex flex-col sm:flex-row justify-between gap-3">
          <span>{copyright}</span>
          <span>
            {settings.footerTagline || "Website Design & Development"}
          </span>
        </div>
      </div>
    </footer>
