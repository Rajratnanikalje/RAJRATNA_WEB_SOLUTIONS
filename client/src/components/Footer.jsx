...
export default function Footer() {
  ...
  const whatsappHref = /^https?:\/\//i.test(whatsappValue) ? whatsappValue : settings.contactWhatsappUrl || (digits ? `https://wa.me/${digits}` : "");
  ...
  return <footer className="border-t border-white/5 pt-14"><div className="container">
    <div className={`grid sm:grid-cols-2 ${gridClass} gap-10`}>
      ...
      {settings.footerQuickLinksVisible !== false && <div><b className="block text-sm font-medium text-slate-400 mb-3">{settings.footerQuickLinksHeading || "Pages"}</b><div className="flex flex-col gap-2 text-sm">{quickLinks.map((item, index) => <Link key={`${item.routeKey}-${index}`} to={publicPath(item.routeKey)} className="text-slate-300 hover:text-white transition-colors">{item.routeKey === "privacy" ? settings.footerPrivacyLabel || item.label : item.routeKey === "terms" ? settings.footerTermsLabel || item.label : item.label}</Link>)}</div></div>}
      {settings.footerServicesVisible !== false && <div><b className="block text-sm font-medium text-slate-400 mb-3">{settings.footerServicesHeading || "What We Build"}</b><div className="flex flex-col gap-2 text-sm text-slate-300">{serviceLinks.map((item, index) => <Link key={`${item.label}-${index}`} to={publicPath(item.routeKey, "/services")} className="hover:text-white">{item.label}</Link>)}</div></div>}
