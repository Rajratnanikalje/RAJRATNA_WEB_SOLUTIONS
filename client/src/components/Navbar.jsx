...
const defaultLinks = [
  ...
  ["process", "Process"],
  ["contact", "Contact"],
].map(([routeKey, label], displayOrder) => ({
  routeKey,
  label,
  visible: true,
  displayOrder,
...
export default function Navbar() {
  ...
  const links = (settings.navbarItems || defaultLinks)
  ...
  const ctaPath = publicPath(
    settings.navbarCtaRouteKey || "contact",
    "/contact",
  );

  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? "bg-[#08111f]/90 backdrop-blur-xl shadow-lg shadow-black/10" : "bg-transparent"}`}
    >
      {settingsError && (
      ...
      <div className="container border-b border-white/5">
        <div className="h-[76px] flex items-center justify-between">
          ...
          <nav
            className="hidden lg:flex gap-8 text-[13px] font-medium text-slate-300"
            ...
            {links.map((item) => {
              ...
              return (
                <NavLink
                  ...
                  onClick={item.routeKey === "home" ? goHome : undefined}
                  className={({ isActive }) =>
                    isActive
                      ? "text-[#78a9ff]"
      ...
      <AnimatePresence>
        {open && (
          ...
          >
            <nav
              className="container relative z-10 py-4 flex flex-col gap-1"
              ...
              {links.map((item) => {
                ...
                return (
                  <Link
                    ...
                    to={to}
                    onClick={
                      item.routeKey === "home" ? goHome : () => setOpen(false)
                    }
