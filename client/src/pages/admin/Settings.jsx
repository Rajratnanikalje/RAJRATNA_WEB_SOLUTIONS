...
const seoPageDefaults = PUBLIC_ROUTE_OPTIONS.map(({ value }) => ({
  pageKey: value,
  indexable: true,
}));
...
export default function Settings({ section = "website" }) {
  ...
  useEffect(() => {
    admin.settings
      ...
      .then((r) => {
        ...
        const pageSeo = seoPageDefaults.map((defaults) => ({
          ...defaults,
          ...(data.pageSeo || []).find(
            (item) => item.pageKey === defaults.pageKey,
          ),
        }));
        setForm({
          ...data,
      ...
      .catch((e) =>
        setError(e.response?.data?.message || "Could not load settings."),
      )
  ...
  const updateSeoPage = (index, key, value) =>
    setForm((current) => ({
      ...current,
      pageSeo: (current.pageSeo || seoPageDefaults).map((page, i) =>
        i === index ? { ...page, [key]: value } : page,
      ),
    }));
  const saveAccount = async (e) => {
    ...
    if (!account.resetToken) {
      setAccountStatus({
        error:
          "Verify the OTP sent to your registered email before updating the account.",
        success: "",
      });
      return;
    }
    try {
      const response = await admin.updateAccount({
        email: account.email,
      ...
      setAccountStatus({
        error: e.response?.data?.message || "Could not update account.",
        success: "",
      });
    }
  };
  const requestAccountOtp = async () => {
    ...
    try {
      ...
      setAccountStatus({
        error: e.response?.data?.message || "Could not send OTP.",
        success: "",
      });
    } finally {
  ...
  const verifyAccountOtp = async () => {
    ...
    try {
      ...
      setAccountStatus({
        error: "",
        success: "OTP verified. You can now update your account.",
      });
    } catch (e) {
      setAccountStatus({
        error: e.response?.data?.message || "Could not verify OTP.",
        success: "",
      });
    } finally {
      setAccountOtpBusy(false);
    ...
    <>
      <div className="mb-8">
        ...
        <p className="text-slate-500 mt-1 text-sm">
          {section === "website"
            ? "Manage the website name and favicon. Navbar branding and logo are managed in Navbar."
            : section === "contact"
              ? "Manage contact details and social links used by the Contact page."
              : "Manage titles, descriptions, canonical URLs, social previews, and indexing per public page."}
        </p>
      </div>
            ...
            {keys.map((k) => (
              <div key={k}>
                ...
                {imageFields[k] ? (
                  ...
                  />
                ) : ["seoDescription", "ogDescription", "socialLinks"].includes(
                    k,
                  ) ? (
                  <textarea
                    className="input"
      ...
      {section === "seo" && (
        <Card className="p-7 mt-6 max-w-4xl">
          ...
          <div className="mb-5">
            <h2 className="text-xl font-semibold">Per-page SEO</h2>
            <p className="text-sm text-slate-500 mt-1">
              Edit metadata for each existing public route. Admin routes remain
              noindex.
            </p>
          </div>
          <div className="grid gap-4">
            {(form.pageSeo || seoPageDefaults).map((page, index) => (
              <details
                key={page.pageKey}
              ...
              >
                <summary className="cursor-pointer font-semibold text-slate-200">
                  {PUBLIC_ROUTE_OPTIONS.find(
                    (route) => route.value === page.pageKey,
                  )?.label || page.pageKey}
                </summary>
                <div className="grid md:grid-cols-2 gap-4 mt-4">
                  {[
                    ...
                    ["canonicalUrl", "Canonical URL (optional)", "url"],
                    ["ogTitle", "Open Graph title", "text"],
                    ["ogDescription", "Open Graph description", "textarea"],
                    ["twitterTitle", "Twitter title", "text"],
                    ["twitterDescription", "Twitter description", "textarea"],
                  ].map(([key, label, kind]) => (
                    <label
                      key={key}
                    ...
                    >
                      ...
                      {kind === "textarea" ? (
                        <textarea
                          ...
                          value={page[key] || ""}
                          onChange={(e) =>
                            updateSeoPage(index, key, e.target.value)
                          }
                        />
                      ) : (
                        <input
                  ...
                  <label className="grid gap-1 text-xs text-slate-400">
                    ...
                    <select
                      className="input"
                      value={page.twitterCard || "summary"}
                      onChange={(e) =>
                        updateSeoPage(index, "twitterCard", e.target.value)
                      }
                    >
                  ...
                  {[
                    ...
                    >
                      ...
                      <ImageUploader
                        value={page[key] || ""}
                        onChange={(url) => updateSeoPage(index, key, url)}
                        previewHeight="h-28"
                        hint="Use the existing Cloudinary uploader."
                      />
                    </label>
                  ))}
                  <label className="flex items-center gap-2 text-sm text-slate-300">
                    <input
                      type="checkbox"
                      checked={page.indexable !== false}
                      onChange={(e) =>
                        updateSeoPage(index, "indexable", e.target.checked)
                      }
                    />{" "}
          ...
          <button
            ...
            onClick={async () => {
              ...
              try {
                await admin.settings.save({
                  pageSeo: form.pageSeo || seoPageDefaults,
                });
                notifyContentUpdated();
                setSuccess("Per-page SEO saved successfully.");
              } catch (e) {
                setError(
                  e.response?.data?.message || "Could not save page SEO.",
                );
              } finally {
                setSaving(false);
              }
            ...
            disabled={saving}
          >
            {saving ? "Saving..." : "Save Per-page SEO"}
            <Save size={15} />
          </button>
        </Card>
      )}
      <Card className="p-7 mt-6 max-w-3xl">
        ...
        <p className="muted text-sm mt-1">
          Verify an OTP sent to your registered email before changing your login
          email or password.
        </p>
        <form onSubmit={saveAccount} className="grid gap-4 mt-5">
          ...
          <input
            className="input"
            type="password"
            placeholder="New password (optional, minimum 8 characters)"
          ...
          {!accountOtpSent ? (
            <button
              ...
              disabled={accountOtpBusy}
            >
              {" "}
              {accountOtpBusy ? "Sending OTP..." : "Send verification OTP"}{" "}
            </button>
          ) : !account.resetToken ? (
            <div className="grid gap-3">
              <input
                className="input tracking-[.35em] text-center"
                inputMode="numeric"
                pattern="[0-9]{6}"
                maxLength={6}
              ...
              <div className="flex gap-3">
                ...
                <button
                  ...
                  onClick={verifyAccountOtp}
                  disabled={accountOtpBusy}
                >
                  {accountOtpBusy ? "Verifying..." : "Verify OTP"}
                </button>
              </div>
            </div>
          ) : null}
          {accountStatus.error && (
            <p className="text-red-300 text-sm">{accountStatus.error}</p>
          )}
          {accountStatus.success && (
            <p className="text-emerald-300 text-sm">{accountStatus.success}</p>
          )}
          {account.resetToken && (
